...
|---|---|---|
| 1 - Update-branch LLM merge | resolved-code | `absorb_batch` update branch now reuses the merge prompt/parser instead of `existing + --- + proposal.body`; W2 proposal/apply was already pre-landed and now has HTTP proposal/apply smoke coverage. Tests: `absorb_batch_update_merges_existing_with_llm`, `proposal_http_smoke_creates_and_applies_merge`. |
| 2 - Bidirectional wikilink maintenance | resolved-visibility | Backlinks/link graph now parse frontend wikilinks `[[slug]]`, `[[slug\|label]]`, and anchors in addition to canonical `concepts/{slug}.md` Markdown links. This closes user-visible bidirectional discovery via backlinks/index/graph; automatic body mutation remains intentionally deferred. Tests: `extract_internal_links_finds_wikilinks`, `list_backlinks_returns_wikilink_referring_pages`, `build_backlinks_index_with_wikilinks`. |
