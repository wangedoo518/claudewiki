    ...
    match message.role.as_str() {
        "assistant" => {
            ...
            if text.is_empty() && tool_calls.is_empty() {
                ...
                // FIX (warwolf vendor patch): omit `tool_calls` entirely
                // when empty. Strict OpenAI-compatible providers like
                // DeepSeek reject `messages[N].tool_calls: []` with
                // `400 Invalid 'messages[N].tool_calls': empty array`.
                // The OpenAI canonical behaviour is to either omit the
                // field or pass null; we do the same for `content`
                // which is already optional via Option<String>.
