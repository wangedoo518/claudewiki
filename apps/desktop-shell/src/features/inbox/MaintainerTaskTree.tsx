/**
 * S0.3 extraction target: Maintainer Task Tree (CCD soul ④).
 *
 * Original: features/session-workbench/SubagentPanel.tsx.
 * Renamed per ClawWiki canonical §6.1. Behavior preserved verbatim;
 * only the component export name and the types import path change.
 * Once S4 lands, the Inbox detail column will swap from "subagent
 * browsing" to "wiki_maintainer action tree" — the UI shape (task list
 * + expand/collapse + tool-call cards + diff preview) stays the same,
 * which is why this extraction is behavior-preserving.
 *
 * Derives subagent state from Agent tool_use / tool_result messages in
...
interface MaintainerTaskTreeProps {
  messages: ConversationMessage[];
  onClose: () => void;
}

export function MaintainerTaskTree({
  messages,
  onClose,
}: MaintainerTaskTreeProps) {
