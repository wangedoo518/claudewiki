...
//
// The Phase 3-5 multi-provider catalogue (`listProviders`,
// `upsertProvider`, `deleteProvider`, `activateProvider`,
// `listProviderTemplates`, `testProvider`, plus the corresponding
// `DesktopProviderSummary` / `UpsertProviderRequest` etc. types) was
// removed per ClawWiki canonical §11.1 cut #6. The Rust backend
// `providers_config` module is gone in the same commit. ClawWiki uses
// a single managed Codex pool from S2 onward — users do not pick a
// provider, do not paste API keys, and do not see this surface.
