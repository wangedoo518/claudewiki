...
WeChat Kefu 是 ClawWiki 的微信消息入口闭环模块。目标形态是将微信客服消息 (URL/文本/文件/?问题) 自动路由到知识管道: URL/文本/文件消息经过 ingest 适配器转为 raw entry 后自动触发 `/absorb`; `?` 前缀问题消息直接路由到 `/query` 返回知识库答案。当前代码已经落地官方 Kefu 配置/监控/管道接口、文本/链接摄入、`?` 查询、`/recent` 与 `/stats` 命令; 文件/图片摄入、共享 `wechat_ingress` 抽象、Settings Modal 迁移仍是后续目标。
...
## 0. P1-3 当前实现快照 (2026-04-15)

本节是代码核对结果, 优先级高于下方仍保留的目标设计段落。

### 0.1 已实现链路

- **个微 iLink 通道**: `rust/crates/desktop-core/src/wechat_ilink/desktop_handler.rs` 仍是个人微信 ClawBot 长轮询路径。它只接收文本; 非文本会回复“暂不支持非文本消息”。当绑定了 `wiki_paths` 时, 文本会写入 raw; 文本中嵌入 URL 时会先走 `wiki_ingest::url::fetch_and_body`, 成功后写 `source="wechat-url"`, 否则降级写 `source="wechat-text"`。随后继续调用 `DesktopState::append_user_message` 走桌面对话回合。
- **官方 Kefu 通道**: `rust/crates/desktop-core/src/wechat_kefu/desktop_handler.rs` 已经实现消息分类与处理: `?`/`？` 进入 `wiki_maintainer::query_wiki`; `/recent`、`/stats` 进入命令回复; 第一条 URL 进入 URL 摄入; 其余文本进入文本摄入。其他 `msgtype` 会回复不支持。
- **Kefu URL 摄入**: URL 通过 `wiki_ingest::url::fetch_and_body` 抓取和校验后, 以 `source="url"` 调用 `wiki_store::write_raw_entry`, 然后通过本机 `POST http://127.0.0.1:4357/api/wiki/absorb` 触发吸收。
- **Kefu 文本摄入**: 普通文本有最小长度门槛, 通过 `RawFrontmatter::for_paste` 与 `write_raw_entry(paths, "wechat-text", slug, text, fm)` 写入 raw, 然后触发同一个本机 absorb。
- **Kefu HTTP/UI 面**: `desktop-server` 已有 `/api/desktop/wechat-kefu/*` 配置、账号、contact-url、status、monitor、callback、pipeline 路由; 前端当前承载在 `apps/desktop-shell/src/features/wechat/WeChatBridgePage.tsx` 的顶层 `#/wechat` 页面。
- **Kefu 能力状态**: `/api/desktop/wechat-kefu/status` 暴露 `capabilities`: `text`、`url`、`query` 为 supported, `commands` 为 `/recent` 与 `/stats`, `file`、`image`、`card`、`share` 为 unsupported。前端据此显示当前能力 badges。

### 0.2 尚未实现或仍是目标设计

- **文件/图片/卡片/分享消息摄入**尚未在 Kefu handler 中落地, 当前只对未知 `msgtype` 返回不支持, status 也显式标记为 unsupported。
- **共享 `wechat_ingress` 抽象**尚未创建; iLink 与 Kefu 现在仍各自有 handler, 不是同一个 channel-agnostic `InboundEnvelope` 管线。
- **Settings Modal WeChat Hub 迁移**尚未完成。`WeChatBridgePage.tsx` 仍是双通道操作页; `apps/desktop-shell/src/features/settings/sections/WeChatSettings.tsx` 目前只管理个微 iLink 账号。
- **`RawEntryMeta` / `KefuIngestResult` / `auto_absorb_enabled`** 这类设计名词没有对应的当前公开代码路径。实际 raw 写入使用 `RawFrontmatter::for_paste` 与 `wiki_store::write_raw_entry(paths, source, slug, body, fm)`。
- **真实设备 E2E**仍需单独验证: 官方客服租户、回调公网地址、WeChat 侧 callback 配置、真实 `open_kfid` contact link 和手机端消息收发不在本次代码 smoke 范围内。

### 0.3 阅读规则

下方章节继续保留目标架构, 用于指导后续迭代。但凡涉及文件/图片摄入、Settings Modal 统一 Hub、新增 `wechat_ingress/*` 抽象、`RawEntryMeta`/`KefuIngestResult` 等内容, 均应按“目标设计/待实现”理解, 不应当被当成当前已上线事实。
...
| 消息分类 | 当前分类: URL / 文本 / `?`问题 / `/`命令; 其他 msgtype 返回不支持 | `wechat_kefu::desktop_handler` |
...
| 文件/图片摄入 | 目标能力; 当前未实现, 未知 msgtype 返回不支持 | 待实现 |
| 自动触发吸收 | 摄入完成后自动 POST `/api/wiki/absorb` 触发 SKILL Engine 吸收 | `wechat_kefu::desktop_handler` |
| ?问题路由 | `?`/`？` 前缀消息 → `wiki_maintainer::query_wiki()` → 格式化答案 | `wechat_kefu::desktop_handler` |
...
| UI 渲染 | `WeChatBridgePage.tsx` / Settings Modal | 当前 Kefu 操作面在 `#/wechat`; Settings Modal 仅管理 iLink |
...
文件消息:   目标合约; 当前 Kefu handler 未实现文件/图片摄入, 会走 unsupported msgtype 回复
...
| PDF 提取 | `wiki_ingest` | `pdf::extract_pdf(path) -> IngestResult` | 目标能力; Kefu 当前未接文件 |
| DOCX 提取 | `wiki_ingest` | `docx::extract_docx(path) -> IngestResult` | 目标能力; Kefu 当前未接文件 |
| 图片准备 | `wiki_ingest` | `image::prepare_image(path) -> IngestResult` | 目标能力; Kefu 当前未接图片 |
| Raw 写入 | `wiki_store` | `write_raw_entry(paths, source, slug, body, fm) -> RawEntry` | 写入 raw/ 目录 |
...
| 消费者 | 层 | 接口 | 用途 |
|--------|-----|------|------|
| 前端 WeChat Hub | `#/wechat` (`WeChatBridgePage.tsx`) | `GET /api/desktop/wechat-kefu/status` | 当前展示连接状态、管道状态和 contact link |
